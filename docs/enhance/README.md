...
| 阶段 2 · 内容质量 | ⬜ 未开始 | 引用、公式、Word 模板、冻数字 |
| 阶段 3 · E2E 评估 | ✅ 完成 | 唯一赛题 2024高教杯C题 + eval 脚本 + scorecard 存档 |
| 阶段 4 · LLM 观测 | ✅ 完成 | `llm.response` trace + eval 聚合 token/延迟 |
| **数据产物先于建模** | ✅ 完成 | 建模前 Coder 清洗 → `data_contract.json` → 建模手读限长文本 → 解题 Coder 重置对话 |

**当前高优（下一波）**：论文实质（冻数字、过程禁令不进正文）→ 图表质量（竞赛图模板、中文图注）。阶段 1 导出格式单测不挡这两波。
